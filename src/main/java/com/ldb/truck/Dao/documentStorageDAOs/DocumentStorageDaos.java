...
public class DocumentStorageDaos implements DocumentInterface {
    ...
    public int InsertDocumentDAOs(DocumentStorageReq documentStorageReq) throws ParseException {
        ...
        try {
            String SQL = "insert into DOCUMENT_STORAGE (PDF,BRANCH_USER,DATECREATE,DOC_TYPE,BRANCH_OFFICE,TOKEN,INBOUNDNUMBER,OUTBOUNDNUMBER,DateExpDoc,CLASSOFDOCS,BOUND,CONTENT_DOC,WHOCARRYDOC,ETC,LEKTEE,DateTakeIn,saveById,company,bouang,inside) value(?,?,now(),'"
                    + documentStorageReq.getDocType() + "','" + documentStorageReq.getBranch() + "','"
                    + documentStorageReq.getToKen() + "','" + documentStorageReq.getInboundnumber() + "','"
                    + documentStorageReq.getOutboundnumber() + "','" + documentStorageReq.getDateExpDoc() + "','"
                    + documentStorageReq.getClassofdocs() + "','" + documentStorageReq.getBound() + "','"
                    + documentStorageReq.getContent_doc() + "','" + documentStorageReq.getWhocarrydoc() + "','"
                    + documentStorageReq.getEtc() + "','" + documentStorageReq.getLektee() + "','"
                    + documentStorageReq.getDatetakein() + "','" + documentStorageReq.getUserId() + "','"
                    + documentStorageReq.getCompany() + "','" + documentStorageReq.getBouang() + "','"
                    + documentStorageReq.getInside() + "')";
            log.info("SQL:" + SQL);
    ...
    // public int InsertMultiPic (DocumentStorageReq documentStorageReq) throws
    // ParseException {
    // String path="http://khounkham.com/images/car/";
    // String fileName = documentStorageReq.getPdf();
    // log.info("path:"+path+fileName);
    // List<DocumentStorageModel> data = new ArrayList<>();
    // try{
    // String SQL = "insert into TB_PICTURE (pic,userId,branch)
    // value(?,'"+documentStorageReq.getBranch()+"','"+documentStorageReq.getBranch()+"')";
    // log.info("SQL:"+SQL);
    // List<Object> paramList = new ArrayList<Object>();
    // paramList.add(path + fileName);
    ...
    public int InsertTaskDaos(TaskReq[] taskReq1) throws ParseException {
        int totalInserted = 0; // Track total inserted records
        try {
            String SQL = "insert into TB_TASKS (TOPIC_TASK,SUB_TASK,START_DATE,END_DATE,toKen,BRANCH_ID,PROGRESS,DESCRIPTION,TIME_TASK) values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
            log.info("Script" + SQL);
            ...
            for (TaskReq taskReq : taskReq1) {
                ...
                paramList.add(taskReq.getDescription());
                paramList.add(taskReq.getTime());
                totalInserted += EBankJdbcTemplate.update(SQL, paramList.toArray());
            }
            return totalInserted;
        } catch (Exception e) {
    ...
    public int UpdateTaskDaos(TaskReq[] taskReq1) throws ParseException {
        int totalInserted = 0; // Track total inserted records
        try {
            String SQL = "update TB_TASKS set TOPIC_TASK=?,SUB_TASK=?, PROGRESS=?, DESCRIPTION=?, TIME_TASK=? where KEY_ID =? ";
            log.info("Script" + SQL);
    ...
    public int DeptMustReceivedInsertDAOs(DeptMustReceivedReq deptMustReceivedReq) throws ParseException {
        ...
        List<DocumentStorageModel> data = new ArrayList<>();
        try {
            sql = "insert into DEBT_MUST_RECEIVED (CUSTOMER_ID,TOPIC,TYPE_ID,CURRENCY,DATE,DUE_DATE,DATE_CREATE,STATUS_WAIT_APPROVE,REFERENCE_NUMBER,LEK_BAI_SUNG,AMOUNT_MONEY,QUOTATION,DOCUMENT_1,DESCRIPTION,NOTE,NUM,UNIT,TOTALMONEY,QUOTATION_CODE,userId,INVOICE_STATUS,ACCOUNT_NUMBER,ACCOUNT_NAME) value(?,?,?,?,?,?,now(),'N','"
                    + deptMustReceivedReq.getReference_number() + "','" + deptMustReceivedReq.getLek_bai_sung() + "','"
                    + deptMustReceivedReq.getAmount_money() + "','" + deptMustReceivedReq.getQuotation() + "','" + d1
                    + "','" + deptMustReceivedReq.getDescription() + "','" + deptMustReceivedReq.getNote() + "','"
                    + deptMustReceivedReq.getNum() + "','" + deptMustReceivedReq.getUnit() + "','"
                    + deptMustReceivedReq.getTotalMooney() + "','" + deptMustReceivedReq.getQuotation_code() + "','"
                    + deptMustReceivedReq.getUserId() + "','N','ACCOUNT_NUMBER','ACCOUNT_NAME') ";
            log.info("SQL:" + sql);
            List<Object> paramList = new ArrayList<Object>();
    ...
    public int DeptMustReceivedInsertDAOsNoDoc(DeptMustReceivedReq deptMustReceivedReq) throws ParseException {
        ...
        List<DocumentStorageModel> data = new ArrayList<>();
        try {
            sql = "insert into DEBT_MUST_RECEIVED (CUSTOMER_ID,TOPIC,TYPE_ID,CURRENCY,DATE,DUE_DATE,DATE_CREATE,STATUS_WAIT_APPROVE,REFERENCE_NUMBER,LEK_BAI_SUNG,AMOUNT_MONEY,QUOTATION,DESCRIPTION,NOTE,NUM,UNIT,TOTALMONEY,QUOTATION_CODE,userId,INVOICE_STATUS,ACCOUNT_NUMBER,ACCOUNT_NAME) value(?,?,?,?,?,?,now(),'N','"
                    + deptMustReceivedReq.getReference_number() + "','" + deptMustReceivedReq.getLek_bai_sung() + "','"
                    + deptMustReceivedReq.getAmount_money() + "','" + deptMustReceivedReq.getQuotation() + "','"
                    + deptMustReceivedReq.getDescription() + "','" + deptMustReceivedReq.getNote() + "','"
                    + deptMustReceivedReq.getNum() + "','" + deptMustReceivedReq.getUnit() + "','"
                    + deptMustReceivedReq.getTotalMooney() + "','" + deptMustReceivedReq.getQuotation_code() + "','"
                    + deptMustReceivedReq.getUserId() + "','N','ACCOUNT_NUMBER','ACCOUNT_NAME') ";
    ...
    public int InvoiceDeptInsertDAOsNoPic(InvoiceDeptReq invoiceDeptReq) throws ParseException {
        ...
        List<DocumentStorageModel> data = new ArrayList<>();
        try {
            String SQL = "insert into INVOICE_DEPT_OUT (date_invoice,detail,GETMONEY_STATUS,AMOUNT_OF_MONEY,userId,quotation_code,INVOICE_CODE) value('"
                    + invoiceDeptReq.getDate_invoice() + "','" + invoiceDeptReq.getDetail() + "','N','"
                    + invoiceDeptReq.getAmount_of_money() + "','" + invoiceDeptReq.getUserId() + "','"
                    + invoiceDeptReq.getQuotation_code() + "','" + invoiceDeptReq.getInvoice_code() + "')";
            log.info("SQL:" + SQL);
            List<Object> paramList = new ArrayList<Object>();
            ...
            // inserrt his payment
            String invoiceHis = "insert into INVOICE_DEPT_HIS (date_invoice,AMOUNT_OF_MONEY,userId,quotation_code,INVOICE_CODE) value('"
                    + invoiceDeptReq.getDate_invoice() + "','" + invoiceDeptReq.getAmount_of_money() + "','"
                    + invoiceDeptReq.getUserId() + "','" + invoiceDeptReq.getQuotation_code() + "','"
                    + invoiceDeptReq.getInvoice_code() + "')";
            ...
            // update amount - amout
            String UpdateAmout = "update DEBT_MUST_RECEIVED set TOTALMONEY=TOTALMONEY-'"
                    + invoiceDeptReq.getAmount_of_money() + "' where QUOTATION_CODE ='"
                    + invoiceDeptReq.getQuotation_code() + "'";
    ...
    public int InvoiceDeptInsertDAOs(InvoiceDeptReq invoiceDeptReq) throws ParseException {
        ...
        List<DocumentStorageModel> data = new ArrayList<>();
        try {
            String SQL = "insert into INVOICE_DEPT_OUT (date_invoice,detail,file,GETMONEY_STATUS,AMOUNT_OF_MONEY,userId,quotation_code,INVOICE_CODE) value('"
                    + invoiceDeptReq.getDate_invoice() + "','" + invoiceDeptReq.getDetail() + "','" + d + "','N','"
                    + invoiceDeptReq.getAmount_of_money() + "','" + invoiceDeptReq.getUserId() + "','"
                    + invoiceDeptReq.getQuotation_code() + "','" + invoiceDeptReq.getInvoice_code() + "')";
            log.info("SQL:" + SQL);
            List<Object> paramList = new ArrayList<Object>();
    ...
    public int DeptMustReceivedUpdateDAOs(DeptMustReceivedReq deptMustReceivedReq) throws ParseException {
        ...
        List<DocumentStorageModel> data = new ArrayList<>();
        try {
            String SQL = "UPDATE DEBT_MUST_RECEIVED SET CUSTOMER_ID=?,TOPIC=?,TYPE_ID=?,CURRENCY=?,DATE=?,DUE_DATE=?,DATE_CREATE=now(),REFERENCE_NUMBER='"
                    + deptMustReceivedReq.getReference_number() + "',LEK_BAI_SUNG='"
                    + deptMustReceivedReq.getLek_bai_sung() + "',AMOUNT_MONEY='" + deptMustReceivedReq.getAmount_money()
                    + "',QUOTATION='" + deptMustReceivedReq.getQuotation() + "',DOCUMENT_1='" + d1 + "',DESCRIPTION='"
                    + deptMustReceivedReq.getDescription() + "',NOTE='" + deptMustReceivedReq.getNote() + "',NUM='"
                    + deptMustReceivedReq.getNum() + "',UNIT='" + deptMustReceivedReq.getUnit() + "',TOTALMONEY='"
                    + deptMustReceivedReq.getTotalMooney() + "' WHERE KEY_ID='" + deptMustReceivedReq.getKey_id() + "'";
            log.info("SQL:" + SQL);
            List<Object> paramList = new ArrayList<Object>();
            paramList.add(deptMustReceivedReq.getCustomer_id());
            paramList.add(deptMustReceivedReq.getTopic());
    ...
    public int InvoiceDeptUpdateDAOs(InvoiceDeptReq invoiceDeptReq) throws ParseException {
        ...
        List<DocumentStorageModel> data = new ArrayList<>();
        try {
            String SQL = "UPDATE INVOICE_DEPT_OUT SET date_invoice=?,detail=?,file=?,AMOUNT_OF_MONEY=?,userId=? WHERE KEY_ID='"
                    + invoiceDeptReq.getKey_id() + "' AND INVOICE_CODE='" + invoiceDeptReq.getInvoice_code() + "'";
            log.info("SQL:" + SQL);
            List<Object> paramList = new ArrayList<Object>();
            paramList.add(invoiceDeptReq.getDate_invoice());
            paramList.add(invoiceDeptReq.getDetail());
            ...
            // update amount - amout
            String UpdateAmout = "update DEBT_MUST_RECEIVED set AMOUNT_MONEY=AMOUNT_MONEY-'"
                    + invoiceDeptReq.getAmount_of_money() + "' where QUOTATION_CODE ='"
                    + invoiceDeptReq.getQuotation_code() + "'";
    ...
    public int UpdateDocumentDAOs(DocumentStorageReq documentStorageReq) throws ParseException {
        ...
        List<DocumentStorageModel> data = new ArrayList<>();
        try {
            String SQL = "update DOCUMENT_STORAGE set DOC_TYPE=?,INBOUNDNUMBER=?,OUTBOUNDNUMBER=?,DateExpDoc=?,CLASSOFDOCS=?,BOUND=?,CONTENT_DOC=?,WHOCARRYDOC=?,ETC=?,LEKTEE=?,DateTakeIn=?,company=?,bouang=?,inside=? where KEY_ID='"
                    + documentStorageReq.getKey_id() + "'";
    ...
    public int InsertDataHoleDAOs(DataHoleReq dataHoleReq) throws ParseException {
        ...
        try {
            String SQL = "insert into DATA_HOLE (pic,hole_number,data_Coller,userId,full_Name_Hole_number,branch_id) value(?,?,?,'"
                    + dataHoleReq.getUserId() + "','" + dataHoleReq.getFull_Name_Hole_number() + "','"
                    + dataHoleReq.getBranch_id() + "')";
            log.info("SQL:" + SQL);
            List<Object> paramList = new ArrayList<Object>();
            paramList.add(path + fileName);
    ...
    public int InsertHeaderTruckFileUpdateDAOs(OnlyFileHeaderTuckReq onlyFileHeaderTuckReq) throws ParseException {
        ...
        List<DataHoleModel> data = new ArrayList<>();
        try {
            String SQL = "update FILE_HEADERTRUCK set HEADERTRUCK_ID=?,FILES=?,date=? where KEY_ID='"
                    + onlyFileHeaderTuckReq.getKey_id_of_file() + "'";
            log.info("SQL:" + SQL);
            List<Object> paramList = new ArrayList<Object>();
            paramList.add(onlyFileHeaderTuckReq.getHeadtruck_id());
            // paramList.add(onlyFileHeaderTuckReq.getUserId());
    ...
    public int AcceptupdateStatusDAOs(DeptMustReceivedReq deptMustReceivedReq) {
        List<BouangModel> result = new ArrayList<>();
        try {
            String SQL = "update DEBT_MUST_RECEIVED set STATUS_WAIT_APPROVE = 'Y' where KEY_ID ='"
                    + deptMustReceivedReq.getKey_id() + "'";
            log.info("sql" + SQL);
            List<Object> paraList = new ArrayList<>();
    ...
    public int InsertResultOfSurveyDAOs(DataHoleReq dataHoleReq) throws ParseException {
        ...
        try {
            String SQL = "insert into RESULT_SURVEY (files,type,branch_of_bor,name,dateInsert,nameDetail,branch_id) value(?,?,'"
                    + dataHoleReq.getBranch() + "',?,?,?,?)";
            log.info("SQL:" + SQL);
            List<Object> paramList = new ArrayList<Object>();
            paramList.add(path + fileName);
    ...
    // public List<DocumentStorageModel> listDocDAOs (DocumentStorageReq
    // documentStorageReq) {
    // String sql;
    // try{
    // if (documentStorageReq.getToKen() != null &&
    // (documentStorageReq.getToKen().equals("UnCuQ8Dql7bSVS9LcDfMWmA8asAtQLMF") ||
    // documentStorageReq.getToKen().equals("KIOMPlY4JcaUE7LZZzlKIKHFSZlTxLue") )){
    // if (documentStorageReq.getBound() != null &&
    // documentStorageReq.getBound().equals("in"))
    // {
    // sql = "select * from V_DOC WHERE BOUND='in'";
    // log.info("SQL in bound:" + sql);
    // }
    // else if (documentStorageReq.getBound() != null &&
    // documentStorageReq.getBound().equals("out"))
    // {
    // sql = "select * from V_DOC WHERE BOUND='"+documentStorageReq.getBound()+"'";
    // log.info("SQL out bound:" + sql);
    // }
    // else if (documentStorageReq.getBound() != null &&
    // documentStorageReq.getBound().equals("inside"))
    // {
    // sql = "select * from V_DOC WHERE inside='inside'";
    // log.info("SQL out bound:" + sql);
    // }
    // else if(documentStorageReq.getUserIdoffinanceial() == null ||
    // documentStorageReq.getUserIdoffinanceial().isEmpty())
    // {
    // sql = "select * from V_DOC";
    //
    // log.info("SQL:" + sql);
    // }else {
    // sql = "select * from V_DOC where
    // saveById='"+documentStorageReq.getUserIdoffinanceial()+"'";
    // log.info("SQL:" + sql);
    // }
    // }
    // else {
    ...
    // {
    // sql = "select * from V_DOC WHERE TOKEN='"+documentStorageReq.getToKen()+"'and
    // BOUND='"+documentStorageReq.getBound()+"'";
    // log.info("SQL in bound:" + sql);
    // }
    // else if (documentStorageReq.getBound().equals("out"))
    // {
    // sql = "select * from V_DOC WHERE TOKEN='" + documentStorageReq.getToKen() +
    // "' and BOUND='"+documentStorageReq.getBound()+"'";
    // log.info("SQL out bound:" + sql);
    // }
    ...
    // @Override
    // public DocumentStorageModel mapRow(ResultSet rs, int rowNum) throws
    // SQLException {
    ...
    public List<InvoiceDeptModel> InvoiceDeptDAOs(DeptMustReceivedReq deptMustReceivedReq) {
        String sql;
        // deptMustReceivedReq.getKey_id() == null ||
        // deptMustReceivedReq.getKey_id().isEmpty()
        try {
            if (deptMustReceivedReq.getToKen().equals("UnCuQ8Dql7bSVS9LcDfMWmA8asAtQLMF")) {
                {
            ...
            // sql = "select * from V_INVOICE_DEPT_MUST_RECIEVED WHERE
            // QUOTATION_CODE_tba='"+deptMustReceivedReq.getQuotation_code()+"'";
            // log.info("SQL:" + sql);
            // }else {
            // sql = "select * from V_INVOICE_DEPT_MUST_RECIEVED WHERE userId='" +
            // deptMustReceivedReq.getUserId() + "'";
            // log.info("SQL:" + sql);
            // }
            // }
    ...
    public List<ListNameModel> ListNameDeptDAOs(DeptMustReceivedReq deptMustReceivedReq) {
        try {
            String sql = "SELECT d.UNIT,l.KEY_ID,l.listName,l.quotation_code,l.amount,price,l.totalPrice,d.TOTALMONEY FROM TB_LIST_DMRC l join DEBT_MUST_RECEIVED d on l.quotation_code =d.QUOTATION_CODE where l.quotation_code = '"
                    + deptMustReceivedReq.getQuotation_code() + "'";
    ...
    public List<CustomerHisPayModel> DeptMustReceivedHistoryCustormerDAOs(DeptMustReceivedReq deptMustReceivedReq) {
        String sql;
        try {
            sql = "select * from V_CUSTOMER_HIS_PAY WHERE userId='" + deptMustReceivedReq.getUserId()
                    + "' and CUSTOMER_ID='" + deptMustReceivedReq.getCustomer_id() + "'";
            log.info("SQL:" + sql);
    ...
    public List<CustomerHisPayModel> SearchDeptMustReceivedHistoryCustormerDAOs(
            DeptMustReceivedReq deptMustReceivedReq) {
        String sql;
        try {
            if (deptMustReceivedReq.getStartDate() == null && deptMustReceivedReq.getEndDate() == null
                    && deptMustReceivedReq.getCustomer_id() != null) {
                ...
                sql = "select * from V_CUSTOMER_HIS_PAY where date_invoice between '"
                        + deptMustReceivedReq.getStartDate() + "' and '" + deptMustReceivedReq.getEndDate() + "'";
                log.info("SQL2:" + sql);
                ...
                sql = "select * from V_CUSTOMER_HIS_PAY where CURRENCY='" + deptMustReceivedReq.getCurrency()
                        + "' and date_invoice between '" + deptMustReceivedReq.getStartDate() + "' and '"
                        + deptMustReceivedReq.getEndDate() + "' and CUSTOMER_ID='"
                        + deptMustReceivedReq.getCustomer_id() + "'";
                log.info("SQL4:" + sql);
            }
            return EBankJdbcTemplate.query(sql, new RowMapper<CustomerHisPayModel>() {
                @Override
        ...
        try {
            if (documentStorageReq.getToKen().equals("UnCuQ8Dql7bSVS9LcDfMWmA8asAtQLMF")
                ...
                if (documentStorageReq.getBouang() != null && documentStorageReq.getCompany() == null
                    ...
                    log.info("SQL in bound:" + sql);
                } else if (documentStorageReq.getBouang() == null && documentStorageReq.getCompany() != null
                        && documentStorageReq.getType() == null) {
                ...
                if (documentStorageReq.getBouang() != null && documentStorageReq.getCompany() == null
                        && documentStorageReq.getType() == null) {
                    sql = "select * from V_DOC WHERE bouang='" + documentStorageReq.getBouang() + "' and TOKEN='"
                            + documentStorageReq.getToKen() + "'";
                    log.info("SQL in bound:" + sql);
                } else if (documentStorageReq.getBouang() == null && documentStorageReq.getCompany() != null
                        && documentStorageReq.getType() == null) {
                    sql = "select * from V_DOC WHERE company='" + documentStorageReq.getCompany() + "'and TOKEN='"
                            + documentStorageReq.getToKen() + "'";
                    log.info("SQL out bound:" + sql);
                } else if (documentStorageReq.getBouang() == null && documentStorageReq.getCompany() == null
                        && documentStorageReq.getType() != null) {
                    sql = "select * from V_DOC WHERE DOC_TYPE='" + documentStorageReq.getType() + "'and TOKEN='"
                            + documentStorageReq.getToKen() + "'";
                    log.info("SQL out bound:" + sql);
                } else {
                    sql = "select * from V_DOC WHERE TOKEN='" + documentStorageReq.getToKen() + "'";
    ...
    public List<TaskModel> AllTaskDAOs(TaskReq taskReq) {
        ...
        if (taskReq.getStartDate() != null && taskReq.getEndDate() != null) {
            ...
            whereClause.append(" START_DATE BETWEEN '").append(taskReq.getStartDate()).append("' AND '")
                    .append(taskReq.getEndDate()).append("'");
        }
    ...
    // try{
    // if (taskReq.getStartDate()== null && taskReq.getEndDate()== null &&
    // taskReq.getStatus() =="all")
    // {
    // sql = "select * from V_TASKS ";
    // }
    // else if (taskReq.getStartDate()== null && taskReq.getEndDate()== null &&
    // taskReq.getStatus() =="not_all")
    // {
    ...
    // taskReq.getStatus() =="not_all")
    // {
    // sql = "select * from V_TASKS where toKen= '"+taskReq.getToKen()+"' AND
    // START_DATE BETWEEN '"+taskReq.getStartDate()+"' AND
    // '"+taskReq.getEndDate()+"'";
    // }
    // else if (taskReq.getStartDate()!= null && taskReq.getEndDate()!= null &&
    // taskReq.getStatus() =="all")
    // {
    // sql = "select * from V_TASKS where START_DATE BETWEEN
    // '"+taskReq.getStartDate()+"' AND '"+taskReq.getEndDate()+"'";
    // }
    // else if (taskReq.getProgress() == null && taskReq.getStartDate()== null &&
    // taskReq.getEndDate()== null && taskReq.getStatus() =="not_all")
    // {
    // sql = "select * from V_TASKS where toKen= '"+taskReq.getToKen()+"'";
    // }
    // else if (taskReq.getProgress() == null && taskReq.getStartDate()== null &&
    // taskReq.getEndDate()== null && taskReq.getStatus() =="all")
    // {
    // sql = "select * from V_TASKS ";
    // }
    // else if (taskReq.getProgress() != null && taskReq.getStartDate()!= null &&
    // taskReq.getEndDate()!= null && taskReq.getStatus() =="all")
    // {
    // sql = "select * from V_TASKS where PROGRESS= '"+taskReq.getProgress()+"' AND
    // START_DATE BETWEEN '"+taskReq.getStartDate()+"' AND
    // '"+taskReq.getEndDate()+"'";
    // }
    // else if (taskReq.getProgress() != null && taskReq.getStartDate()!= null &&
    ...
    // sql = "select * from V_TASKS where PROGRESS= '"+taskReq.getProgress()+"' AND
    // START_DATE BETWEEN '"+taskReq.getStartDate()+"' AND
    // '"+taskReq.getEndDate()+"' AND toKen= '"+taskReq.getToKen()+"'";
    // }
    // else if (taskReq.getProgress() != null && taskReq.getStartDate()== null &&
    // taskReq.getEndDate()== null && taskReq.getStatus()=="all")
    // {
    ...
    // {
    // sql = "select * from V_TASKS where PROGRESS= '"+taskReq.getProgress()+"' AND
    // toKen= '"+taskReq.getToKen()+"'";
    // }
    // else
    // {
    // sql = "select * from V_TASKS where toKen= '"+taskReq.getToKen()+"' AND
    // START_DATE BETWEEN '"+taskReq.getStartDate()+"' AND
    // '"+taskReq.getEndDate()+"' and PROGRESS= '"+taskReq.getProgress()+"'";
    // }
    //
    // return EBankJdbcTemplate.query(sql, new RowMapper<TaskModel>() {
    // @Override
    ...
    public List<PicOfBorModel> PicOfSurveyDAOs(DataHoleReq dataHoleReq) {
        ...
        try {
            ...
            if (dataHoleReq.getFolderName() == null && dataHoleReq.getStartDate() == null
                    && dataHoleReq.getEndDate() == null) {
                sql = "select * from TB_PICTURE where branch_id='" + dataHoleReq.getBranch_id() + "'";
            } else if (dataHoleReq.getFolderName() == null && dataHoleReq.getStartDate() != null
                    && dataHoleReq.getEndDate() != null) {
                sql = "select * from TB_PICTURE where branch_id='" + dataHoleReq.getBranch_id()
                        + "' AND dateCreate between '" + dataHoleReq.getStartDate() + "' AND '"
                        + dataHoleReq.getEndDate() + "' ";
            } else {
                sql = "select * from TB_PICTURE where branch_id='" + dataHoleReq.getBranch_id() + "' AND folderName = '"
                        + dataHoleReq.getFolderName() + "' AND dateCreate between '" + dataHoleReq.getStartDate()
                        + "' AND '" + dataHoleReq.getEndDate() + "' ";
            }
    ...
    // public List<DocumentStorageModel> listDocumentAdmin (DocumentStorageReq
    // documentStorageReq) {
    // try{
