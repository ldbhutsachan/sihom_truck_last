...
public interface OrderAuthEntityRepository extends CrudRepository<OrderAuthEntity,Long> {
    ...
    @Query(value = "select * from v_order_item where status not in ('wait','auth') order by status desc ", nativeQuery = true)
    List<OrderAuthEntity> getOrderAuthByBuyer();

    @Query(value = "select * from v_order_item where status not in ('wait','auth','buyer') order by status desc ", nativeQuery = true)
