...
public class DocumentStorageController {
    ...
    public Messages InsertDocument(@RequestParam("files") MultipartFile[] files, @RequestParam("toKen") String toKen,
            @RequestParam("folderName") String folderName, @RequestParam("dateCreate") String dateCreate,
    ...
    public ResponseEntity<?> createPDF(
            @ApiParam(name = "Body Request", value = "JSON body request to check information", required = true) @Valid @PathVariable(name = "fileName") String fileName,
        ...
        try {
            ...
            // header.add(HttpHeaders.CONTENT_DISPOSITION, "inline; filename=" + fullPath);
            // // User for View file
            ...
            // initializeMessage.errorMessage("File image not found !!", "System can not
            // generate file")
            // );
