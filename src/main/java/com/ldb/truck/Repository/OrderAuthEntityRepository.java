...
public interface OrderAuthEntityRepository extends CrudRepository<OrderAuthEntity,Long> {
    @Query(value = "select * from v_order_item where branchno=:branchno and status in ('auth','wait') order by status desc ", nativeQuery = true)
    List<OrderAuthEntity> getOrderAuthByBranchNo(@Param("branchno") String branchno);

    @Query(value = "select * from v_order_item where status  in ('wait-item','auth') order by status desc ", nativeQuery = true)
