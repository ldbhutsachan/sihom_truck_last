...
public interface OrderTxnEntityRepository extends CrudRepository<OrderItemEntity,Long> {
    ...
    List<OrderItemEntity> getOrderBySaveby(@Param("saveby") String saveby,@Param("status") String status);
    ...
    List<OrderItemEntity> getOrderByBillNo(@Param("saveby") String saveby,@Param("billNo") String billNo,@Param("status") String status);
   ...
   // @Query(value = "select * from v_order_item where status in ('wait','wait-order') order by detail_id desc ", nativeQuery = true)
    ...
    List<OrderItemEntity> getOrderByAdmin(@Param("status") String status);
